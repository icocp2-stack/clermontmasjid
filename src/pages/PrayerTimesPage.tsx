...
export default function PrayerTimesPage() {
  ...
  const [adminPrayerTimes, setAdminPrayerTimes] = useState<AdminPrayerTimes | undefined>(undefined);
  ...
  const loadAdminPrayerTimes = async () => {
    ...
    if (data) {
      const base = new Date();
      setAdminPrayerTimes({
        fajr: data.fajr_time ? parseTimeToDate(data.fajr_time, base) : undefined,
        dhuhr: data.dhuhr_time ? parseTimeToDate(data.dhuhr_time, base) : undefined,
        asr: data.asr_time ? parseTimeToDate(data.asr_time, base) : undefined,
        maghrib: data.maghrib_time ? parseTimeToDate(data.maghrib_time, base) : undefined,
        isha: data.isha_time ? parseTimeToDate(data.isha_time, base) : undefined,
        tahajjud: data.tahajjud ? parseTimeToDate(data.tahajjud, base) : undefined,
      });
    } else {
      setAdminPrayerTimes(undefined);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      <div className="container mx-auto px-3 py-4 md:px-4 md:py-6">
        ...
        <Navigation activeView={activeView} onViewChange={setActiveView} currentPage="prayer-times" />
            ...
            <div className="lg:col-span-2 flex flex-col">
              <div className="bg-slate-900 rounded-lg shadow-2xl border border-slate-800 overflow-hidden w-full aspect-square">
                <PrayerClock
                  ...
                  city={location.city}
                  adminPrayerTimes={adminPrayerTimes}
                />
              </div>
              <PrayerTimesDisplay
